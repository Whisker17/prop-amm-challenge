...
| 005-vol-adaptive-cpmm-fee | 005 Vol-Adaptive CPMM Fee | source (Rust, direct submission shape) — `dcccrypto/percolator-perp-liquidity`'s `EdgeMax_CumVar.rs` | fitted (WHI-1209) — beats the 0-line | `FEE_LO=5, A_NUM=13, B_DEN=1265` — `FEE_LO` sits on its own frozen lower bound, `NOTES.md` § Search; avg edge 422.93, seeds `0..=999`, 10,000 steps, native (train 430.79, validation 425.95); 27-cell grid vs. `001`: 17/27 cells favor `005` (`results/2026-08-21-grid.md`, explained in `NOTES.md` § Grid mode) |
