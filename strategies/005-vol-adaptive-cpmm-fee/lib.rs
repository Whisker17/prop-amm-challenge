...
const NAME: &str = "005 Vol-Adaptive CPMM Fee";
// Preserved unchanged from `docs/references/005-vol-adaptive-cpmm-fee/EdgeMax_CumVar.rs`
...
// previous step boundary exactly there (one clean sample per step). This is
// only conditionally true — at low sigma the arbitrageur often doesn't trade a
// given step at all, so the first executed trade of that step is retail
// instead, whose post-trade reserves also carry that trade's own price impact
// (see NOTES.md § Estimator bias for the expected consequence in low-vol
// regimes; this does not change the mechanism, only its interpretation).
//
...
const WARMUP_STEPS: u64 = 16; // per-step samples required before trusting sigma_hat — frozen
...
fn fee_from_state(_side: u8, _rx: u128, _ry: u128, storage: &[u8]) -> u128 {
    ...
    if magic != MAGIC || count < WARMUP_STEPS {
        // Cold start / warmup: not enough samples to trust the estimate yet. This
        // also sanitises garbage/random storage bytes (crates/cli/src/commands/
        // validate.rs's randomized probe): a random 8-byte magic essentially
        // never collides with MAGIC, so any unrecognised storage falls back here
        // rather than reading var_sum/sample_count as arbitrary large integers.
        return COLD_FEE;
    }
