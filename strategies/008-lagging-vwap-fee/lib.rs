...
const NAME: &str = "008 Lagging-VWAP Directional Fee";
// Preserved unchanged from `docs/references/008-lagging-vwap-fee/strategy.rs` (see
// NOTES.md § Provenance): the compute_swap/after_swap mechanism is a verbatim port, so the
// field describing which model produced that mechanism hasn't changed either. `NAME` is
// this project's own registry label, not the source's own submission name — WHI-1236 Step
// 0(b) leaves verbatim submission of the artifact itself undecided, so this entry is
// identified as our own internal candidate, never the source author's.
const MODEL_USED: &str = "Opus 4.6";

// ============================================================================
// Lagging-VWAP directional fee + 3-profile ensemble.
//
// Source: houseofjiao/prop-amm-challenge @ 152697153d (docs/references/008-lagging-vwap-fee/
// strategy.rs, verbatim). No LICENSE on that repo or upstream — copied for internal
// analysis only per WHI-1236 Step 0(a); not to be submitted as our own entry (Step 0(b)).
//
// MECHANISM (from source, not from memory — see NOTES.md § Mechanism for the full
// derivation). A lagging Kalman-filtered trade VWAP (`p_hat`, blended with a faster EMA
// `p_hat_fast` under high shock) stands in for the fair price the interface does not
// supply. Each quote is classified by direction against that reference: pushing `spot`
// further away is presumptively informed and gets a `+arb_k*dev^2` surcharge; moving
// toward it is presumptively uninformed and gets a `-counter_k*(...)*dev^2` rebate. A
// per-trade ratchet (`fee_floor`, decaying between steps) tracks realized
// volatility/toxicity/shock/size, and a 3-profile ensemble (balanced / max-defense /
// loss-minimizer) picks which `(arb_k, counter_k, size_k, shock_read_k, default_fee,
// tox_read_k)` tuple prices the *next* quote, chosen by an EMA'd counterfactual-edge score
// with switch hysteresis.
//
// SHAPE SAFETY — within one `compute_swap` call, `fee` is a pure function of storage and
// constants (state is read, never written); only the size-penalty term depends on `input`,
// and `d/dinput[input^2/(input+R)] in [0,1)` with `S - f >= 0.9*S > size_k` bounds keeps the
// output monotone and concave — see NOTES.md § Shape risk for the full argument, including
// the one named residual (the size-penalty overflow fallback's formula switch near
// input ~8.8e14 nano), which `bench fuzz` below is the gate for, not a source rewrite.
//
// PARITY — `handle_after_swap` decodes storage from the instruction, runs the SAME
// `after_swap` on a local buffer, then persists via `set_storage`, so native and BPF fee
// trajectories are identical (same pattern `004`/`007` use).
// ============================================================================
...
// [152..160) step_vol      (u64) : per-step input-volume accumulator
// [160..168) step_dir_vol  (u64) : unused by this file's own logic (reserved by source)
// [168..176) step_shock_cum(u64) : unused by this file's own logic (reserved by source)
// [176..184) step_tox_max  (u64) : unused by this file's own logic (reserved by source)
// [184..192) FEE_1         (u64, 1e9) : mirrors stored_fee for profile 1
// [192..200) FEE_2         (u64, 1e9) : mirrors stored_fee for profile 2
...
// Dead in the source itself (declared, never read or written anywhere) — kept verbatim
// per §2.9 rather than dropped; see the P1_*/P2_* dead-constant note below.
...
// ---- The frozen search space (docs/DESIGN.md §6.2, WHI-1236) ----
// All four are the source's own profile-0 constants, active ~85% of the time by the
// source's own measurement (non-citable per WHI-1236's evidential rule — the *fraction*
// is a third-party report, not evidence used here; the choice of which four constants to
// free is this issue's own reasoning). Bounds: `27_333`/`4_600` are the source's own
// profile-2 (loss-minimizer) values for the same constants — the widest the source author
// ever ran them; `4_350` is the source's own profile-1 value; `4..=80` mirrors `004`'s own
// anchored base-fee range. Every other constant in this file (profiles 1/2, tox/shock read
// weights, decay rates, Kalman/switcher thresholds) is frozen at the source's own value —
// see NOTES.md § Parameter space for why these four and not others.
// === PARAMS BEGIN ===
...
// Source note, preserved: all three profiles share this one target-fee formula (they
// differ only in `quote_profile` behavior via `profile_params` below) — `after_swap`
// (below) calls `make_target` with only the P0_* set. The P1_*/P2_* sets are therefore
// dead in the source itself (never read anywhere), and are kept verbatim rather than
// dropped, per §2.9's "faithfully first" — this file inherits the source's own
// `#![allow(dead_code)]`.
const P0_BASE_FEE: u128 = TARGET_BASE_BPS as u128 * BPS;
...
// Adaptive score decay: faster after shocks, stable in calm
...
// Score biases per profile (base values). P0=balanced, P1=max defense, P2=loss minimizer
...
// Asymmetric: faster downward than upward.
...
const KALMAN_INNO_THRESH: u128 = 10_000_000; // 1% innovation gating
                                             // Source's own name and value, kept verbatim (§2.9 fidelity contract): the Kalman gain's
                                             // fallback/floor value, despite the generic name.
const ALPHA: u128 = 20_000_000;

// ---- Profile params (quote_profile) ----
const SIGMA_CLASS_THRESH: u128 = 5_000_000; // 0.5%

// Profile 0 (balanced) — arb_k/counter_k/size_k are this file's free PARAMS above.
...
fn profile_params(profile: u8, _sigma_class: u8) -> (u128, u128, u128, u128, u128, u128, bool) {
    let (spot_arb_k, spot_counter_k, size_live_k, shock_read_k, default_fee, tox_read_k) =
        match profile {
            0 => (
                ARB_K_BPS as u128 * BPS,
                COUNTER_K_BPS as u128 * BPS,
                SIZE_K_BPS as u128 * BPS,
                P0_SHOCK_K,
                55 * BPS,
                74 * BPS,
            ),
...
fn quote_profile(
    ...
    let mut fee = if init_flag == 0 {
        ...
        if risk_gate_counter {
            ...
            let bump = if is_arb_dir {
                wmul(spot_arb_k, dev2)
            } else {
                0
            };
...
fn raw_edge_sample(
    ...
    let vol_t = if shock_pre >= BIAS_SHOCK_THRESH {
        SCALE
    } else {
        wdiv(shock_pre, BIAS_SHOCK_THRESH)
    };
...
fn score_one_profile(
    ...
    // Adaptive decay based on shock level
    let shock_t = if shock_pre >= SHOCK_DECAY_THRESH {
        SCALE
    } else {
        wdiv(shock_pre, SHOCK_DECAY_THRESH)
    };
...
pub fn compute_swap(data: &[u8]) -> u64 {
    ...
    let sigma_class = if sigma >= SIGMA_CLASS_THRESH {
        1u8
    } else {
        0u8
    };
    ...
    let mut mode = if init_flag == 0 {
        0
    } else {
        rd64(storage, S_MODE) as u8
    };
...
pub fn after_swap(data: &[u8], storage: &mut [u8]) {
    ...
    if init_flag != 0 {
        ...
        if pre_ok && rx_pre > 0 && ry_pre > 0 {
            ...
            let w_pre = if sh_pre > 300_000_000 {
                300_000_000
            } else {
                sh_pre
            };
            ...
            let is_arb_pre =
                (spot_pre > p_ref_pre && side == 0) || (spot_pre < p_ref_pre && side == 1);
            ...
            let sigma_class_pre = if sigma_pre >= SIGMA_CLASS_THRESH {
                1u8
            } else {
                0u8
            };
            ...
            if best_mode != mode && step.saturating_sub(last_switch) >= SWITCH_COOLDOWN {
                ...
                let abs_gap = if best_mode > mode {
                    SWITCH_ABS_GAP_UP as i128
                } else {
                    SWITCH_ABS_GAP_DOWN as i128
                };
    ...
    if new_step {
        ...
        if init_flag != 0 {
            ...
            let mut ema =
                wmul(cnt_ema as u128, CNT_EMA_DECAY) + cnt as u128 * (SCALE - CNT_EMA_DECAY);
        ...
        if sum_w > 0 && init_flag != 0 {
            ...
            let inno = if p_hat > 0 {
                wdiv(abs_diff(vwap, p_hat), p_hat)
            } else {
                0
            };
        ...
        let elapsed = if init_flag == 0 {
            1
        } else {
            (step - last_step).min(ELAPSED_CAP)
        };
        let _fd = if shock_hat > 15_000_000 {
            FLOOR_DECAY_HI
        } else {
            FLOOR_DECAY
        };
    ...
    let ret = if p_hat_fast > 0 {
        wdiv(abs_diff(p_impl, p_hat_fast), p_hat_fast)
    } else {
        0
    };
    ...
    if first {
        ...
        let vd = if step < EARLY_STEP_THRESH {
            VOL_DECAY_FAST
        } else {
            VOL_DECAY
        };
    ...
    let tox = if p_hat_fast > 0 {
        wdiv(abs_diff(spot, p_hat_fast), p_hat_fast)
    } else {
        0
    };
    ...
    let reserve_in = if side == 0 { ry } else { rx };
    let trade_frac = if reserve_in > 0 {
        wdiv(input, reserve_in)
    } else {
        0
    };
    ...
    let _sv_frac = if reserve_in > 0 {
        wdiv(step_vol, reserve_in)
    } else {
        0
    };
    let tox_shock_spike = shock_hat > 15_000_000 && tox > 15_000_000;
    let sv_bump = if _sv_frac > 300_000_000 {
        10 * BPS
    } else if _sv_frac > 150_000_000 {
        5 * BPS
    } else {
        0
    };
