...
| --- | --- | --- |
| C-orbic-oracle | Orbic Oracle Re-anchor | A concentrated, spread-bearing Orbic-family curve quoting off a trade-triggered replay of the GBM fair-price path, vs. `001-cpmm-fee` (the 0-line) — `C-orbic-oracle/NOTES.md`. WHI-1248 added a second, exact-step "fingerprint" cursor mode alongside the trade-triggered one; its own hardening checks first caught a structural flaw in the reconstruction (floor-clamping in the arbitrageur's search made step-matching non-injective), closing its `L=1`/`L=0` rungs as a documented negative result. WHI-1250's buy-probe-only cursor fix and pre-registered kill rule bounded that flaw well enough to report both rungs as survivors-only means, each under 1% exclusion (`L=1`/`validation` +161.4688, n=991/1000; `L=0`/`validation` +400.2951 diff / 802.284247 raw, n=999/1000, sitting below its own closed-form envelope of 1502.562225 — a retail-flow-only bound, not a strictly like-for-like comparison against the simulated figure) — see `NOTES.md` § WHI-1248 for the original finding and § WHI-1250 for the fix, its own scope caveats, and the measured outcome. |
