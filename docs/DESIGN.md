...
built on top of it (`PAMM-002` → `PAMM-003` → `PAMM-004`). The ordering is the point:
`PAMM-002` ends by cross-checking bench against the upstream CLI, and starter's 210.50 is
...
2. **bench and its own tests can be wrong together.** Both are ours. *Mitigation:* the
   starter 210.50 per-seed cross-check (`PAMM-002`) and the single-peak self-check on the
   fee family (`PAMM-003`) — two independent anchors produced by upstream code paths.
