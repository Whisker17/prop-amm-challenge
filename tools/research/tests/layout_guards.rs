...
fn tools_research_has_no_submission_shaped_lib_rs() {
    ...
    while let Some(dir) = stack.pop() {
        for entry in std::fs::read_dir(&dir).unwrap_or_else(|e| {
            ...
            let path = entry.path();
            if path.is_dir() {
                // Skip only the crate-root target/ (a nested CARGO_TARGET_DIR).
                // Workspace builds land in repo-root target/; a directory
                // named target/ deeper in the source tree is still walked.
                if path.file_name().and_then(|n| n.to_str()) == Some("target")
                    && path.parent() == Some(research.as_path())
                {
                    continue;
                }
                stack.push(path);
