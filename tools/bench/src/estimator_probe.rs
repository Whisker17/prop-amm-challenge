...
//! `docs/DESIGN.md`'s architecture-risk table and `docs/DEFERRED_ISSUES.md`) — not a refactor
//! of either `lib.rs`:
//!
...
//!
//! `set_floor_bps_sweep`/[`FLOOR_BPS_SWEEP`]: the floor sweep is a protocol-level tunable
//! (`config/bench.toml`'s `[estimator_probe]` table, docs/DESIGN.md §3.1), not a Rust
//! constant — but `ewma004_recorder` is a bare `fn` pointer (`AfterSwapFn` has no room for
//! captured state, the same constraint `compile.rs`/`telemetry.rs` document for their own
//! installed-fn statics), so the sweep is threaded in via a process-global set once,
//! single-threaded, before each parallel batch — the exact same pattern this module's own
//! `REAL_AFTER_SWAP_*` statics already use for the real `after_swap` fn pointer.
...
// tracked in `docs/DEFERRED_ISSUES.md`). ----
const P_SCALE_005: u128 = 1_000_000_000;
const MOVE_BPS_CAP_005: u128 = 250;
// Derived, mirroring 005's own `R2_CAP` — by construction this is `MOVE_BPS_CAP_005^2`, the
// cap on one squared per-step move.
const R2_CAP_005: u128 = MOVE_BPS_CAP_005 * MOVE_BPS_CAP_005;
// Mirrors 005's own `after_swap` guard (`data.len() < 42 || storage.len() < STATE_END`) — its
// `STATE_END` is 56 there; this probe's shadow state needs the same 42-byte payload prefix
// `decode_after_swap` reads from (`side, input_amount, output_amount, reserve_x, reserve_y,
// step`), so the length half of that guard is what applies here (there is no probe-side
// storage buffer to floor-check).
const AFTER_SWAP_PAYLOAD_MIN_LEN: usize = 42;
...
/// The skeleton every single-AMM shadow probe shares: install `after_swap_fn` into `slot` for
/// `recorder` to delegate to, then run `configs` through `recorder` on a bounded rayon pool,
/// collecting one `T` per simulation via `finalize` — the real edge trajectory is untouched
/// (delegate-then-observe, proven per-probe by this module's own
/// `*_probe_does_not_perturb_the_real_run` tests).
///
/// Callers MUST hold the probe's own `CALL_LOCK_*` for their **entire** call into this
/// function, not just around it — `slot` is process-global (two concurrent calls installing
/// different `after_swap_fn`s would otherwise race), and `run_004_floor_probe` additionally
/// writes a second process-global (`FLOOR_BPS_SWEEP`) that must stay atomic with the batch run
/// it configures. Taking the lock *inside* this shared function would only protect `slot`,
/// not a caller's own writes made just before calling it — which is exactly the race an
/// earlier version of this code had (WHI-1225 round-1 review): `FLOOR_BPS_SWEEP` was written
/// before the lock was acquired, so two concurrent `run_004_floor_probe` calls (e.g. two
/// `cargo test` threads) could interleave their writes and each run against the *other's*
/// floor sweep.
fn run_single_amm_probe<T: Send>(
    slot: &AtomicPtr<()>,
    swap_fn: SwapFn,
    after_swap_fn: Option<AfterSwapFn>,
    recorder: AfterSwapFn,
    configs: &[SimulationConfig],
    finalize: impl Fn(&SimulationConfig) -> anyhow::Result<T> + Sync,
) -> anyhow::Result<Vec<T>> {
...
/// including its redundant-but-defensive `R2_CAP` re-clamp, but tracking `count` and
/// `elapsed_sum` (`step - last_step`) side by side so both normalizations can be computed from
/// one run.
fn vol005_recorder(data: &[u8], storage: &mut [u8]) {
    if data.len() >= AFTER_SWAP_PAYLOAD_MIN_LEN {
        let (_side, _input_amount, _output_amount, rx, ry, step, _storage) =
            ...
            VOL005_STATE.with(|s| {
                ...
                if !st.initialized {
                    ...
                    if last_price > 0 {
                        ...
                        let mut r2 = move_bps.saturating_mul(move_bps);
                        if r2 > R2_CAP_005 {
                            r2 = R2_CAP_005; // redundant given the move clamp, mirrors 005's own hard ceiling
                        }
...
/// The `ewma_vol`-vs-floor sweep, in bps — set once, single-threaded, before each
/// [`run_004_floor_probe`] call (guarded by `CALL_LOCK_004`), then read-only for the duration
/// of that call's parallel batch. See this module's own doc comment for why a process-global
/// is needed here (`ewma004_recorder` is a bare `fn` pointer with no captured state).
static FLOOR_BPS_SWEEP: RwLock<Vec<u64>> = RwLock::new(Vec::new());
...
struct Ewma004State {
    ...
    last_ry: u64,
    ewma_vol: u64,
    below_floor_steps: Vec<u64>,
    total_steps: u64,
    below_floor_volume: Vec<f64>,
    total_volume: f64,
}
...
fn take_ewma004_state(n_floors: usize) -> Ewma004State {
    EWMA004_STATE.with(|s| {
        let state = s.borrow().clone();
        *s.borrow_mut() = Ewma004State {
            below_floor_steps: vec![0; n_floors],
            below_floor_volume: vec![0.0; n_floors],
            ..Ewma004State::default()
        };
        state
    })
}
...
/// [`FLOOR_BPS_SWEEP`] threshold, both by trade count and by this trade's own Y-volume (via
/// `telemetry::volume_y`, the same convention `telemetry.rs::record_and_delegate` uses).
fn ewma004_recorder(data: &[u8], storage: &mut [u8]) {
    if data.len() >= AFTER_SWAP_PAYLOAD_MIN_LEN {
        ...
        let floors = FLOOR_BPS_SWEEP.read().unwrap_or_else(|p| p.into_inner());
        EWMA004_STATE.with(|s| {
            let mut st = s.borrow_mut();
            // Resize (not just "if empty") — a thread reused across two `run_004_floor_probe`
            // calls with a *different-length* sweep would otherwise keep the previous call's
            // vector size and panic indexing past it below.
            if st.below_floor_steps.len() != floors.len() {
                st.below_floor_steps = vec![0; floors.len()];
                st.below_floor_volume = vec![0.0; floors.len()];
            }
            ...
            st.last_ry = cur_ry;
            st.total_steps += 1;
            st.total_volume += this_trade_volume_y;
            for (i, &floor_bps) in floors.iter().enumerate() {
                if new_vol <= floor_bps * 100_000 {
                    st.below_floor_steps[i] += 1;
                    st.below_floor_volume[i] += this_trade_volume_y;
                }
            }
        });
    }
...
/// answered here per WHI-1225's "Added scope"). `below_floor_steps`/`below_floor_volume` are
/// indexed the same as the `floor_bps` slice passed to [`run_004_floor_probe`] — a per-trade
/// count and Y-volume respectively, not a per-simulation-step count (`004` updates its EWMA on
/// every executed submission trade, with no notion of "one sample per step" the way `005`
/// has — a step where the router sent the submission no flow contributes to neither).
#[derive(Debug, Clone, PartialEq)]
pub struct Ewma004ProbeSim {
    pub seed: u64,
    pub true_sigma: f64,
    pub below_floor_steps: Vec<u64>,
    pub total_steps: u64,
    pub below_floor_volume: Vec<f64>,
    pub total_volume: f64,
}

/// Runs the committed `004` over `configs`, alongside the shadow EWMA above, sweeping
/// `floor_bps` (config-driven — `config/bench.toml`'s `[estimator_probe]` table, docs/DESIGN.md
/// §3.1 — not a Rust constant).
pub fn run_004_floor_probe(
    ...
    // Written while holding `_guard`, for the same reason `run_single_amm_probe`'s own doc
    // comment explains: this must stay atomic with the batch run below, or a concurrent call
    // could interleave its write between this one and the run it's meant to configure.
    {
        let mut sweep = FLOOR_BPS_SWEEP.write().unwrap_or_else(|p| p.into_inner());
        *sweep = floor_bps.to_vec();
    }
    let n_floors = floor_bps.len();
    ...
    run_single_amm_probe(
        ...
        configs,
        move |config| {
            let state = take_ewma004_state(n_floors);
            Ok(Ewma004ProbeSim {
                seed: config.seed,
                true_sigma: config.gbm_sigma,
...
mod tests {
    ...
    const TEST_FLOOR_BPS: [u64; 7] = [0, 10, 20, 25, 30, 40, 60];
    ...
    fn ewma004_probe_does_not_perturb_the_real_run() {
        ...
        let probe_sims =
            run_004_floor_probe(normalizer::compute_swap, None, &configs, &TEST_FLOOR_BPS).unwrap();
    ...
    fn ewma004_probe_records_every_trade_and_a_monotone_floor_ladder() {
        ...
        for sim in &sims {
            ...
            assert_eq!(sim.below_floor_steps.len(), TEST_FLOOR_BPS.len());
            // A higher floor can only ever count at least as many steps below it.
            for i in 1..TEST_FLOOR_BPS.len() {
    ...
    #[test]
    fn ewma004_probe_honours_a_different_floor_sweep_on_a_later_call() {
        // Two consecutive calls with different-length sweeps must each get their own sweep,
        // not a stale one left over from the previous call's process-global.
        let configs = tiny_configs(2);
        let small_sweep = [30u64];
        let sims_small =
            run_004_floor_probe(normalizer::compute_swap, None, &configs, &small_sweep).unwrap();
        for sim in &sims_small {
            assert_eq!(sim.below_floor_steps.len(), 1);
        }
