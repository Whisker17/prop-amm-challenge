...
import "vendor/univ2periphery/UniswapV2Library.sol";
...
/// @notice Uniswap V2 algorithm cell, CANONICAL 30 bps fee.
///
/// `UniswapV2Library.getAmountOut` is the pinned upstream quote:
...
contract UniV2Cell {
    ...
    {
        if (zeroForOne) {
            return UniswapV2Library.getAmountOut(amountIn, state[0], state[1]);
        }
        return UniswapV2Library.getAmountOut(amountIn, state[1], state[0]);
    }
    ...
    function _swap(bool zeroForOne, uint256 amountIn) private returns (uint256 amountOut) {
        if (zeroForOne) {
            amountOut = UniswapV2Library.getAmountOut(amountIn, _s[0], _s[1]);
            _s[0] = _s[0] + amountIn;
            _s[1] = _s[1] - amountOut;
        } else {
            amountOut = UniswapV2Library.getAmountOut(amountIn, _s[1], _s[0]);
            _s[1] = _s[1] + amountIn;
            _s[0] = _s[0] - amountOut;
        }
    }
